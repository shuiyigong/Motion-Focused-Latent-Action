...
class SelfAttention(nn.Module):
    def __init__(self, model_dim: int, num_heads: int, dropout: float = 0.0, rot_emb: bool = False, return_attention: bool = False) -> None:
    ...
    def scaled_dot_product_attention(
        ...
        if is_causal:
            ...
            attn_weight.masked_fill_(causal_mask.logical_not(), float("-inf"))

        if attn_mask is not None:
            mask_condition = (attn_mask == 0) 
            attn_weight.masked_fill_(mask_condition, float("-inf"))
    ...
    def forward(self, x: Tensor, is_causal: bool = False, attn_mask: Tensor = None) -> Tensor:
        ...
        out_data = self.scaled_dot_product_attention(q, k, v, is_causal=is_causal, attn_mask=attn_mask)
        
        if self.return_attention:
            out, attn_weights = out_data
...
class SpatioTemporalBlock(nn.Module):
    def __init__(self, model_dim: int, num_heads: int, dropout: float = 0.0,return_attention: bool = False) -> None:
        super(SpatioTemporalBlock, self).__init__()
        self.spatial_attn = SelfAttention(model_dim, num_heads, dropout=dropout, return_attention=return_attention)
        self.temporal_attn = SelfAttention(model_dim, num_heads, dropout=dropout, return_attention=False)
    ...
    def forward(self, x: Tensor, causal_temporal: bool = False, attn_mask: Tensor = None) -> Tensor:
        ...
        x_ = self.norm1(x)
        if self.return_attention:
            x_attn, attn_weights = self.spatial_attn(x_, is_causal=False, attn_mask=attn_mask)
            self.last_attn_weights = attn_weights
        else:
            x_attn = self.spatial_attn(x_, is_causal=False, attn_mask=attn_mask)
            self.last_attn_weights = None
        x_ = x_attn
        x = x + x_
...
class SpatioTemporalTransformer(nn.Module):
    ...
    def forward(self, x: Tensor, lang_embed: Tensor = None, attn_mask: Tensor = None) -> Tensor:
        ...
        for block in self.transformer_blocks:
            ...
            #print("Block Attention Weights:", block.last_attn_weights)
...
class SpatioBlock(nn.Module):
    ...
    def forward(self, x: Tensor, attn_mask: Tensor = None, return_attention: bool = False) -> Tensor:
        ...
        attn_weights = None
        if self.return_attention:
            x_attn, attn_weights = self.spatial_attn(x_, attn_mask=attn_mask)
        else:
            x_attn = self.spatial_attn(x_, attn_mask=attn_mask)
...
class SpatioTransformer(nn.Module):
    ...
    def forward(self, x: Tensor, lang_embed: Tensor = None, attn_mask: Tensor = None, return_attention: bool = False) -> Tensor:
        x = self.ffn(x)
        x = self.pos_enc(x)
        ...
        for block in self.transformer_blocks:
            if return_attention:
                # 每个 block 都返回 attention
                x, attn_weights = block(x, attn_mask=attn_mask, return_attention=True)
                all_attn_weights.append(attn_weights)
            else:
                x = block(x, attn_mask=attn_mask, return_attention=False)
        if return_attention:
            last_attn_weights = all_attn_weights[-1]
        

        x = self.out(x)
        
        if return_attention:
            return x, last_attn_weights, all_attn_weights
        return x
