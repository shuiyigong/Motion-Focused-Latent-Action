...
from experiments.robot.openvla_utils import get_processor,crop_and_resize
...
class GenerateConfig:
    ...
    pretrained_checkpoint: Union[str, Path] = "your_checkpoint.pt"  # Pretrained checkpoint path
    load_in_4bit: bool = False                       # (For OpenVLA only) Load with 4-bit quantization
    load_in_8bit: bool = False
    action_decoder_path:str = "your_action_decoder.pt"  # Pretrained action decoder checkpoint path
    center_crop: bool = True                         # Center crop? (if trained w/ random crop image aug)
    save_video: bool = True                         # Whether to save rollout videos
    run_name: str = "your_run_name"            # Run name for logging

    using_dino: bool = True                    # Whether the model uses DINO visual features
    dino_proj_ckpt: str = "your_dino_proj_ckpt.pt" # Checkpoint path for linear projection layer that maps DINO features to model visual embedding space (only needed if using_dino==True)
    #################################################################################################################
...
def eval_libero(cfg: GenerateConfig) -> None:
    ...
    if cfg.using_dino:
        dino_proj = nn.Linear(768, 4096)
        dino_proj.load_state_dict(torch.load(cfg.dino_proj_ckpt))
        dino_proj.eval().cuda()
    ...
    if cfg.using_dino:
                ...
                dino_encoder = torch.hub.load('facebookresearch/dinov2', 'dinov2_vitb14_reg').eval().cuda()
    for task_id in tqdm.tqdm(range(num_tasks_in_suite)):
        ...
        for episode_idx in tqdm.tqdm(range(cfg.num_trials_per_task)):
            ...
            while t < max_steps + cfg.num_steps_wait and jieshu == False:
                try:
                    ...
                    if cfg.using_dino:
                        with torch.no_grad():
                            ...
                            dino_input = dino_transform(torch.tensor(image).permute(2,0,1).unsqueeze(0).float()/255.).cuda()
                            dino_features = dino_encoder.forward_features(dino_input)
                            dino_patches = dino_features["x_norm_patchtokens"]
                            visual_embed = dino_proj(dino_patches)
            ...
            if cfg.save_video:
                ...
                save_rollout_video(
                    replay_images, total_episodes, success=done, task_description=task_description, log_file=log_file,savename=cfg.run_name
                )
