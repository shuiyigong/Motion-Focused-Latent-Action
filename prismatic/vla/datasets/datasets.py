...
'austin_sailor_dataset_converted_externally_to_rlds', 
'toto', 'viola', 'droid','furniture_bench_dataset_converted_externally_to_rlds']
...
class RLDSBatchTransformLIBERO_withHis:
    ...
    def __call__(self, rlds_batch: Dict[str, Any]) -> Dict[str, Any]:
        ...
        pixel_values_fordino = self.image_transform_lam(input_img) 

        if "image_wrist" in rlds_batch["observation"]:
            wrist = Image.fromarray(rlds_batch["observation"]["image_wrist"][0].squeeze(-1))#.copy()
            wrist_pixel_values = self.image_transform(wrist)
            wrist_pixel_values_fordino = self.image_transform_lam(wrist)
        ...
        return dict(pixel_values=pixel_values, input_ids=input_ids, labels=labels, actions=rlds_batch["action"][randomized_overlap: self.window_size + randomized_overlap], latent_action_idx=latent_action_idx, dataset_name=dataset_name,
                    pixel_values_fordino=pixel_values_fordino, wrist_pixel_values=wrist_pixel_values, wrist_pixel_values_fordino=wrist_pixel_values_fordino)
...
class RLDSBatchTransformLIBERO:
    ...
    def __call__(self, rlds_batch: Dict[str, Any]) -> Dict[str, Any]:
        ...
        input_img = Image.fromarray(rlds_batch["observation"]["image_primary"][0])
        pixel_values = self.image_transform(input_img)
        pixel_values_fordino = self.image_transform_lam(input_img)
...
class RLDSBatchTransformVideo:
    ...
    def __call__(self, rlds_batch: Dict[str, Any]) -> Dict[str, Any]:
        ...
        mask = Image.fromarray(rlds_batch["observation"]["image_mask"][0].squeeze(-1))#.copy()
        initial_mask_values = self.image_transform(mask)
        ...
        mask_k = Image.fromarray(rlds_batch["observation"]["image_mask"][target_frame_index].squeeze(-1))
        # print(sum(np.array(img_k) - np.array(img)))
        target_pixel_values= self.image_transform(img_k)
        target_mask_values = self.image_transform(mask_k)

        return dict(initial_pixel_values=initial_pixel_values, target_pixel_values=target_pixel_values, 
                    initial_mask_values=initial_mask_values, target_mask_values=target_mask_values,
...
class RLDSDataset(IterableDataset):
    def __init__(
        ...
        per_dataset_kwargs, weights = get_oxe_dataset_kwargs_and_weights(
            self.data_root_dir,
            mixture_spec,
            load_camera_views=("primary","mask"),
            load_depth=False,
            load_proprio=False,
            load_language=True,
        ...
        if image_aug:
            ...
            rlds_config["frame_transform_kwargs"].update({
                "image_augment_kwargs": {
                    "primary": rgb_augment_kwargs,
                    #"mask": mask_augment_kwargs, #if using mask
                    #"wrist": {}, #if using wrist
                }
            })
