...
class PaddedCollatorForActionPrediction_LIBERO:
    ...
    def __call__(self, instances: Sequence[Dict[str, torch.Tensor]]) -> Dict[str, torch.Tensor]:
        input_ids, labels = tuple([instance[key] for instance in instances] for key in ("input_ids", "labels"))
        pixel_values = [instance["pixel_values"] for instance in instances]
        pixel_values_fordino = [instance["pixel_values_fordino"] for instance in instances]
        if "wrist_pixel_values" in instances[0]:
            wrist_pixel_values = [instance["wrist_pixel_values"] for instance in instances]
            wrist_pixel_values_fordino = [instance["wrist_pixel_values_fordino"] for instance in instances]
        ...
        if isinstance(pixel_values[0], torch.Tensor):
            ...
            raise ValueError(f"Unsupported `pixel_values` type = {type(pixel_values)}")
        if isinstance(pixel_values_fordino[0], torch.Tensor):
            pixel_values_fordino = torch.stack(pixel_values_fordino)
        elif isinstance(pixel_values_fordino[0], dict):
            pixel_values_fordino = {
                k: torch.stack([pixel_values_fordino[idx][k] for idx in range(len(input_ids))]) for k in pixel_values_fordino[0]
            }
        else:
            raise ValueError(f"Unsupported `pixel_values_fordino` type = {type(pixel_values_fordino[0])}")
        if "wrist_pixel_values" in instances[0]:
            if isinstance(wrist_pixel_values[0], torch.Tensor):
                wrist_pixel_values = torch.stack(wrist_pixel_values)
            elif isinstance(wrist_pixel_values[0], dict):
                wrist_pixel_values = {
                    k: torch.stack([wrist_pixel_values[idx][k] for idx in range(len(input_ids))]) for k in wrist_pixel_values[0]
                }
            else:
                raise ValueError(f"Unsupported `wrist_pixel_values` type = {type(wrist_pixel_values[0])}")
            if isinstance(wrist_pixel_values_fordino[0], torch.Tensor):
                wrist_pixel_values_fordino = torch.stack(wrist_pixel_values_fordino)
            elif isinstance(wrist_pixel_values_fordino[0], dict):
                wrist_pixel_values_fordino = {
                    k: torch.stack([wrist_pixel_values_fordino[idx][k] for idx in range(len(input_ids))]) for k in wrist_pixel_values_fordino[0]
                }
            else:
                raise ValueError(f"Unsupported `wrist_pixel_values_fordino` type = {type(wrist_pixel_values_fordino[0])}")
        output = dict(
            pixel_values=pixel_values,
            input_ids=input_ids,
            attention_mask=attention_mask,
            ...
            pixel_values_fordino = pixel_values_fordino,
            wrist_pixel_values=wrist_pixel_values,
            wrist_pixel_values_fordino=wrist_pixel_values_fordino
        )
        if dataset_names is not None:
            output["dataset_names"] = dataset_names
        return output
...
class CollatorForLatentAction:
    ...
    def __call__(self, instances: Sequence[Dict[str, torch.Tensor]]) -> Dict[str, torch.Tensor]:
        ...
        initial_mask_values = [instance["initial_mask_values"] for instance in instances]
        initial_mask_values = torch.stack(initial_mask_values)

        
        target_pixel_values = [instance["target_pixel_values"] for instance in instances]
        target_pixel_values = torch.stack(target_pixel_values)

        target_mask_values = [instance["target_mask_values"] for instance in instances]
        target_mask_values = torch.stack(target_mask_values)



        pixel_values = torch.stack([initial_pixel_values, target_pixel_values], dim=1)
        pixel_mask_values = torch.stack([initial_mask_values, target_mask_values], dim=1)
