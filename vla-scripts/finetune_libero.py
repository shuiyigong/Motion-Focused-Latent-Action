...
class Wrapped_Model(torch.nn.Module):
    ...
    def action_decoder_forward(self, batch, vla_output):
        wrist_dino = True
        if wrist_dino:
            wrist_pixel_values = batch["wrist_pixel_values_fordino"]  # [B, 3, H, W]
            wrist_pixel_values = self.dino_transform(wrist_pixel_values)
            
            with torch.no_grad():
                wrist_dino_out = self.dino_encoder.forward_features(wrist_pixel_values)
                wrist_dino_patches = wrist_dino_out["x_norm_patchtokens"]  # [B, 256, 768]
            visual_embed = vla_output.hidden_states[-1][:, : self.vla.vision_backbone.featurizer.patch_embed.num_patches ].to(torch.float)

            wrist_visual_embed = self.dino_proj(wrist_dino_patches).to(torch.float)
            visual_embed = torch.cat([visual_embed, wrist_visual_embed], dim=1)
        else:
            pixel_values = batch["pixel_values_fordino"]  # [B, 3, H, W]
            pixel_values = self.dino_transform(pixel_values)
            
            with torch.no_grad():
                dino_out = self.dino_encoder.forward_features(pixel_values)
                dino_patches = dino_out["x_norm_patchtokens"]  # [B, 256, 768]

            visual_embed = self.dino_proj(dino_patches).to(torch.float)
...
class FinetuneConfig:
    # fmt: off
    vla_path: str = "/mnt/public/user/MFVLA/models/triple_bridgeall"            # Path to your local VLA path
    lam_path: str = "/mnt/public/user/MFVLA/logs/checkpoints/lam2-bridgeall/epoch=1-step=15000.ckpt"#"/mnt/public/user/MFVLA/logs/checkpoints/lam2_triple_path/epoch=3-step=10000.ckpt"
    # Directory Paths
    data_root_dir: Path = Path("/mnt/public/user/hf-cache/hub/datasets--openvla--modified_libero_rlds/snapshots/6ce6aaaaabdbe590b1eef5cd29c0d33f14a08551")      # Path to Open-X dataset directory
    dataset_name: str = "libero_10_no_noops"                   # Name of fine-tuning dataset (e.g., `droid_wipe`)
    run_root_dir: Path = Path("/mnt/public/user/MFVLA/libero_finetune_runs_long_triple_bridgeall_freeze")                               # Path to directory to store logs & checkpoints
    adapter_tmp_dir: Path = Path("/mnt/public/user/MFVLA/libero_finetune_adapter-tmplong_triple_bridgeall_freeze")                     # Temporary directory for LoRA weights before fusing
    resume: bool = False
    actiondecoder_path: str = "/mnt/public/user/MFVLA/libero_finetune_runs_long/triple_path_bridge+libero_long_no_noops+b32+lr-0.00035+lora-r32+dropout-0.0--image_aug=w-LowLevelDecoder-ws-12-dino/action_decoder-10000.pt"
    ...
    # Tracking Parameters
    wandb_project: str = "finetune-LIBERO-long"                          # Name of W&B project to log to (use default!)
...
def finetune(cfg: FinetuneConfig) -> None:
    ...
    exp_id += f'=w-LowLevelDecoder-ws-{cfg.window_size}-dino'
    ...
    #wrapped_model = Wrapped_Model_FlowMatching(vla = vla, freeze_vla = cfg.freeze_vla, window_size=cfg.window_size).to(device_id)
    if cfg.resume:
        wrapped_model.action_decoder.load_state_dict(torch.load(cfg.actiondecoder_path))
    ...
    with tqdm.tqdm(total=cfg.max_steps, leave=False) as progress:
        ...
        for batch_idx, batch in enumerate(dataloader):
            ...
            batch["pixel_values_fordino"] = batch["pixel_values_fordino"].to(torch.bfloat16).to(device_id)
            batch["wrist_pixel_values"] = batch["wrist_pixel_values"].to(torch.bfloat16).to(device_id)
            batch["wrist_pixel_values_fordino"] = batch["wrist_pixel_values_fordino"].to(torch.bfloat16).to(device_id)
            ...
            if gradient_step_idx > 0 and gradient_step_idx % cfg.save_steps == 0:
                if distributed_state.is_main_process:
                    ...
                    torch.save(wrapped_model.module.dino_proj.state_dict(), str(run_dir) + f'/dino_proj-{gradient_step_idx}.pt')
