...
class HFConvertConfig:
    ...
    openvla_model_path_or_id: Union[str, Path] = (                      # Path to Pretrained VLA (on disk or HF Hub)
        "/MFVLA/vla_pretraining_log/prism-dinosiglip-224px+mx-bridge+n0+b32+x42_bridgeall_whole--image_aug-Latent-Action-Pretraining-bridgeall"
    )
    ckpt_name = "step-020000-epoch-03-loss=0.9845.pt"                # The specific checkpoint to be converted (modify accordingly)
    output_hf_model_local_path: Path = Path(                            # Path to Local Path to save HF model
        "/MFVLA/models/bridgeall_whole"
    )
...
def convert_openvla_weights_to_hf(cfg: HFConvertConfig) -> None:
    ...
    tokenizer = AutoTokenizer.from_pretrained(
        'hf-cache/transformers/models--meta-llama--Llama-2-7b-hf/snapshots/01c7f73d771dfac7d292323805ebc428287df4f9', model_max_length=hf_config.llm_max_length, token=cfg.hf_token, padding_side="right"
    )
    ...
    for idx, timm_model_id in enumerate(hf_config.timm_model_ids):
        if 'dino' in timm_model_id:
            pretrained_cfg={'file': 'hf-cache/hub/models--timm--vit_large_patch14_reg4_dinov2.lvd142m/snapshots/f3c408e77602bb412aa65fb03dfa0d5f95cb3832/model.safetensors'}
        else:
            pretrained_cfg={'file': 'hf-cache/hub/models--timm--ViT-SO400M-14-SigLIP/snapshots/9179d15177ece40964c50492136eda2f3e0c9f61/open_clip_model.safetensors'}
